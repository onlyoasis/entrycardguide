---
title: "土耳其 eVisa 官网是哪个？认准 evisa.gov.tr"
kicker: "有些中介把签证卖给本来免签的旅客。付款前先查外交部名单，再逐字核对域名。"
description: "辨认土耳其 eVisa 官网：只认 evisa.gov.tr，先查中国等护照免签，并对比 91 至 213 美元的已记录中介报价。"
date: 2026-07-14
lastmod: 2026-07-14
country: "turkey"
weight: 30
keywords: ["土耳其 eVisa 官网", "土耳其电子签官网", "iVisa 是土耳其官网吗", "evisa.gov.tr", "土耳其签证骗局", "evisa.govn.tr"]
layout: is-ivisa
---

{{< official-link site="turkey.mfa_visa_info" >}}

## 直接答案

土耳其政府 e-Visa 申请网站只有 `evisa.gov.tr`。iVisa 和其他收费代办都是商业公司，不是土耳其外交部。

中国普通护照从 2026 年 1 月 2 日起，旅游访问可在任意 180 日内免签停留最多 90 日。美国、英国、加拿大和澳大利亚普通护照当前也适用 90/180 免签。向这些免签旅客继续卖 e-Visa，卖的是本次行程根本不存在的需求。

{{< official-link site="turkey.evisa" >}}

## 从域名末尾开始看

官方主机名是 <mark>evisa.gov.tr</mark>，结尾必须是 **`.gov.tr`**。

`evisa.govn.tr` 多了一个字母 `n`，`gov` 变成了 `govn`，因此不是政府主机名。2026 年 7 月 14 日，该近似域名把自己包装成土耳其共和国 e-Visa 门户，并对美、英护照使用了过时的资格说法。

域名中出现 Turkey、visa、official、government、portal 或 service，都不能证明它属于政府。先打开上面的外交部或电子签入口，再逐字对比地址栏，确认后才输入护照和银行卡资料。

## 4 种已记录卖法

2026 年 7 月 14 日公开页面显示：

- **turkey-visa.org**：标准服务费 91 美元，紧急服务费 114 美元，另列 13 至 99 美元政府费，页面总价为 104 至 213 美元。
- **visasforms.com**：自称 “Official Turkey eVisa Service”，标出含政府费的 95 美元服务，后文又承认旅客可以直接走政府网站。
- **turkey.gwsg.org**：列出 119 和 120 美元的旅游签产品，未含 VAT，也没有先把免签护照排除。
- **evisa.govn.tr**：使用近似官方域名，并用过时的美、英护照资格说法推销授权。

这些数字只记录各站自己的公开报价，不是政府费用。官网费用按护照而定，完成资格选择后才显示。

## 已记录的商业站点

下方卡片只读取 `data/official_urls/turkey.toml` 保存的证据。

<div class="grid grid-cols-1 md:grid-cols-2 gap-4 my-8">
{{< scam-site country="turkey" domain="evisa.govn.tr" >}}
{{< scam-site country="turkey" domain="turkey-visa.org" >}}
{{< scam-site country="turkey" domain="turkey.gwsg.org" >}}
{{< scam-site country="turkey" domain="visasforms.com" >}}
</div>

## 已经付款怎么办

先查外交部页面。护照原本免签时，要求卖家说明究竟交付了什么服务并申请退款，同时保存结账页、收据、邮件和免签证据。

护照确实需要 e-Visa 时：

- 找到官方申请编号和签发的 PDF；
- 确认邮箱和申请编号由自己掌握；
- 对照护照检查姓名、旅行证件号和有效期；
- 再次付款前，先用 `evisa.gov.tr` 的继续申请入口查询；
- 没有可用结果时，向发卡行说明服务未交付或描述有误。

商业网站的确认页不能证明土耳其外交部已经签发 e-Visa。
