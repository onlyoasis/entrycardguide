---
title: "Japan"
kicker: "Official Visit Japan Web link, free QR workflow, and how to avoid fake login pages."
description: "Guide to Visit Japan Web: official Digital Agency URL, free arrival QR workflow, fields, and middleman warnings."
---

Visit Japan Web is Japan's official digital service for arrival-related QR workflows. The stable government service page is on the Digital Agency domain.

{{< official-link site="japan.vjw" >}}

## Start here

### 1. Visit Japan Web, explained
What it does, what it does not do, and why the `.go.jp` source matters.
→ [Read the overview](/japan/visit-japan-web/)

### 2. Every field, in order
Passport, flight, arrival date, account email, and QR fallback notes.
→ [Field-by-field guide](/japan/how-to-fill/)

### 3. Is iVisa official?
No. Visa services and Visit Japan Web are separate.
→ [iVisa official-site check](/japan/is-ivisa-official/)

## The fast facts

- **Official source:** Digital Agency service page under `.go.jp`.
- **Cost:** free.
- **What it is for:** arrival, customs, and related QR workflows.
- **What it is not:** a visa approval or immigration status guarantee.

## Who we are

We are not Japan's Digital Agency or Immigration Services Agency. We do not collect passport data or create Visit Japan Web accounts.
