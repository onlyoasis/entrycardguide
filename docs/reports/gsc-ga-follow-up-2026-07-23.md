...
**数据可靠性提醒**：`assets/js/analytics.ts` 的 gtag 代理直到 `52d084d`（2026-07-15 合并）之前都在往 dataLayer 推**数组**而不是 `arguments` 对象，GA4 不保证正确处理。本节窗口是 6/25–7/22，**前三周落在修复之前**，所以事件数和「关键事件无数据」这两项要打折看。下一轮复盘应取 7/15 之后的干净窗口重测。
...
### P1：修 GA4 收入追踪（只剩后台配置）

代码侧已经修好了：`52d084d`（2026-07-15）把 gtag 代理从推数组改成推 `arguments`，`scripts/check-seo-output.mjs` 也加了断言守住它，不会再退化。

**剩下的是 GA4 后台动作**：在 property `p536218791` 里把 `affiliate_click` 标记为「关键事件」，否则无法量化 SafetyWing/Airalo 点击。这批 AI 流量能不能变现，取决于先能不能测到。
